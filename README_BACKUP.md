...
## 기타 작업 목록
### 메인화면
1) 로딩 중 Indicator 추가하기[O]
2) 화면 상단 글쓰기, 프로필 버튼 우측으로 정렬[O]
3) Footer 하단 픽스[O]
4) 버튼 누르면 offset 추가 로딩 기능 구현
### 등록화면
1) layout 잡기[O]
2) 제목 / 내용 / 위도, 경도 / 파일선택 / 버튼 순으로 정렬[O]
3) 글쓰기 완료 후 fragment 추가[O]
4) 등록한 사진 미리보기 기능 추가 
### 상세화면
1) layout 잡기[O]
2) 버튼 레이아웃, 디자인 설정[O]
3) 사진 / 제목 / 작성자 / 날짜 / 내용 순으로 정렬[O]
4) 목록 기능 추가[O]
5) 날짜 보여지는 거 수정하기
### 수정화면
1) layout 잡기
